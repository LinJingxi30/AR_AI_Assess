...
class TimedChallengeMode:
    ...
    def get_sket_list(self, image, use_flip=False):
        ...
        imageSket = self.pose_detector.findPose(image, draw=True)
        sketList, bndboxDict = self.pose_detector.findPosition(imageSket, draw=True)
        # print(_, file=sys.stderr)
        return sketList, bndboxDict
    ...
    def main_loop(self):
        ...
        while self.running:
            ...
            # 实时骨架检测
            sketList, bndboxDict = self.get_sket_list(image, use_flip=False)
            # {'bbox': (94, 165, 421, 1126), 'center': (304, 728)}
            bbox = bndboxDict['bbox']
            x_min, y_min, x_max, y_max = bbox
            masked_image = np.zeros_like(image)
            # 添加边距（扩大一圈）
            margin = 100  # 边距大小，可以根据需要调整
            x_min = max(0, x_min - 20)
            x_max = min(WIN_WIDTH, x_max + 20)
            y_min = max(0, y_min - margin)
            y_max = min(WIN_HEIGHT, y_max + margin)
            ...
            new_x_max = min(WIN_WIDTH, new_x_max)
            new_y_min = max(0, new_y_min)
            new_y_max = min(WIN_HEIGHT, new_y_max)
            ...
            # 再跑一次
            sketList, _ = self.get_sket_list(masked_image, use_flip=False)
    ...
    def main_update(self):
        if self.running:
            ...
            bbox = bndboxDict['bbox']
            x_min, y_min, x_max, y_max = bbox
            masked_image = np.zeros((WIN_HEIGHT, WIN_WIDTH, 3), dtype=np.uint8)
            # 添加边距（扩大一圈）
            margin = 100  # 边距大小，可以根据需要调整
            x_min = max(0, x_min - 20)
            x_max = min(WIN_WIDTH, x_max + 20)
            ...
            # 将原始边界框区域复制到新的中心位置
            bbox_content = image[y_min:y_max, x_min:x_max]
            print(shape(masked_image), file=sys.stderr)
