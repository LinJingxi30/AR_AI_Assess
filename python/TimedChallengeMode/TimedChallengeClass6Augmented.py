...
class TimedChallengeMode:
    ...
    def display_pose_from_camera(self, image):
        ...
        with self.mp_pose.Pose(
            ...
            if results.pose_landmarks:
                ...
                try:
                    resized_bbox = cv2.resize(bbox_content, (x_max - x_min, y_max - y_min))
                    masked_image[y_min:y_max, x_min:x_max] = resized_bbox
                except Exception as e:
                    print(f"调整边界框大小失败: {e}", file=sys.stderr)
    ...
    def main_update(self):
        if self.running:
            ...
            # image = self.display_pose_from_camera(image)
