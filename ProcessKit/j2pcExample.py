...
"""
Json2PreviewClass.py 库使用说明 (as ProcessKit)

0. 读取json文件，所有帧存入frames：
    使用 ProcessKit.get_json_frames(...) 方法

1. 指定坐标位置绘制骨架：
    ProcessKit.draw_pose_at_pos(...)

2. 指定起始终止坐标位置、提前多少秒，生成预览坐标：
    先初始化预览坐标生成器：get_coords = ProcessKit.PreviewCoordsGenerator(...) 使用get_coords接方法
    然后在主循环中使用 get_coords.get_preview_coords_only(...) 获取预览坐标，返回值为两个骨架的坐标
    最后使用 ProcessKit.draw_preview_area(...) 绘制预览区域
"""
